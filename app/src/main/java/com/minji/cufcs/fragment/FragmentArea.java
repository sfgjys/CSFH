...
import com.minji.cufcs.adapter.AreaAdapter;
...
public class FragmentArea extends BaseFragment<AreaSingle> implements
		OnClickListener, Observers {

	private List<AreaSingle> singleList;
	...
	private ArrayList<AreaSingle> middleList = new ArrayList<AreaSingle>();
	...
	protected View onCreateSuccessView() {
		...
		areaAdapter = new AreaAdapter(singleList);
		...
		listView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				if (position == singleList.size()) {
					...
					areaAdapter.loadMore();
				} else {

					stationname = singleList.get(position).getStationname(); // 用于页面标题
					...
					rdsid = singleList.get(position).getRdsid(); // 用于查看信息
		...
		try {
			...
			if (obj.has("rows")) {
				...
				for (int i = 0; i < rows.length(); i++) {
					...
					AreaSingle areaSingle = new AreaSingle();
			...
			if (singleList == null) {
				singleList = new ArrayList<AreaSingle>();
			}
			singleList.clear();
	...
	public void onClick(View v) {
		...
		case R.id.bt_make_sure:
			...
			if ("已实施未完成".equals(state)) {// 此处需要请求网络获取信息
				if (!StringUtils.isEmpty(implementId)
						&& !StringUtils.isEmpty(rdsid)
						&& !StringUtils.isEmpty(stationid)) {// 三个id不为空
					alertDialog.cancel();
	...
	private void requestImformation() {
		List<BasicNameValuePair> list = new ArrayList<BasicNameValuePair>();
		list.add(new BasicNameValuePair("rdid", implementId));
		list.add(new BasicNameValuePair("sid", "" + stationid));
		list.add(new BasicNameValuePair("rdstationid", rdsid));
		try {
			String address = SharedPreferencesUtil.getString(
					ViewsUitls.getContext(), "address", "");
			String mImformation = httpBasic.postBack(address
					+ ApiField.INFORMATION, list);
	...
	private void thisToImplementActivity() {
		...
		intent.putExtra("rdsid", rdsid); // rdsid一种特殊id

		intent.putExtra("areaOrautonomously", 1); // 区分片区与自主
	...
	public void onDestroy() {
		FragmentArea area = (FragmentArea) FragmentFactory.fragments[0];

		MySubject.getInstance().del(area);

		FragmentFactory.fragments[0] = null;
